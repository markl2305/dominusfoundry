...
(() => {
  ...
  root.innerHTML = `<link rel="stylesheet" href="${new URL('sabina-chat.css',assetBase)}">
    <button class="launcher" aria-label="Ask Sabina — help and support" aria-haspopup="dialog"><span class="label">Ask Sabina</span><img class="orb" alt="" width="76" height="76"></button>
    <dialog class="panel" aria-labelledby="chat-title"><header class="header"><img alt="" width="42" height="42"><div><h2 id="chat-title">Ask Sabina</h2><p>Help, support, and questions</p></div><button class="close" aria-label="Close chat">×</button></header>
  ...
  async function send(text,retry=false){
    ...
    busy=true;failed=null;$('.error').hidden=true;$('.progress').hidden=false;$('.welcome').hidden=true;
    ...
    finally{clearTimeout(timer);controller=null;busy=false;$('.progress').hidden=true;controls();conversation.scrollTop=conversation.scrollHeight;if(dialog.open)input.focus();}
  }
